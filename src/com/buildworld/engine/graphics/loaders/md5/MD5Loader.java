package com.buildworld.engine.graphics.loaders.md5;
...
public class MD5Loader {
    ...
    private static Mesh createMesh(List<AnimVertex> vertices, List<Integer> indices) {
        ...
        float[] positionsArr = FileUtils.listToArray(positions);
        float[] textCoordsArr = FileUtils.listToArray(textCoords);
        float[] normalsArr = FileUtils.listToArray(normals);
        int[] indicesArr = FileUtils.listIntToArray(indices);
        int[] jointIndicesArr = FileUtils.listIntToArray(jointIndices);
        float[] weightsArr = FileUtils.listToArray(weights);
    ...
    private static void handleTexture(Mesh mesh, MD5Mesh md5Mesh, Vector4f defaultColour) throws Exception {
        ...
        if (texturePath != null && texturePath.length() > 0) {
            ...
            if (pos > 0) {
                ...
                if (FileUtils.existsResourceFile(normalMapFileName)) {
