...
import com.buildworld.engine.graphics.migrate.bootstrap.*;
import com.buildworld.engine.graphics.colors.RGBAColor;
import com.buildworld.engine.graphics.migrate.text.Font;
import org.lwjgl.glfw.GLFW;
import org.lwjgl.system.MemoryStack;
import org.lwjgl.system.MemoryUtil;

import com.buildworld.engine.math.*;
...
public class Renderer3D  implements Renderer {
    ...
    public void drawTexture(Texture texture, float x, float y, float z) {
        drawTexture(texture, x, y, z, RGBAColor.WHITE);
    }
     ...
     */
    public void drawTexture(Texture texture, float x, float y, float z, RGBAColor c) {
        /* Vertex positions */
        float x1 = x;
        float y1 = y;
        ...
        drawTextureRegion(x1, y1, z, x2, y2, z, s1, t1, s2, t2, c);
    }
     ...
     * @param c         TheRGBAColor to use
     */
    public void drawTextureRegion(Texture texture, float x, float y, float z, float regX, float regY, float regWidth, float regHeight, RGBAColor c) {
        /* Vertex positions */
        float x1 = x;
        float y1 = y;
        float x2 = x + regWidth;
     ...
     * @param c  TheRGBAColor to use
     */
    public void drawTextureRegion(float x1, float y1, float z1, float x2, float y2, float z2, float s1, float t1, float s2, float t2, RGBAColor c) {
        if (vertices.remaining() < 8 * 6) { // Drawing 6 vertices, and each vertex has 8 bytes associated to it as shown in the bottom most function in this file
        ...
        // TODO: This is where meshes should do their magic

        vertices.put(x1).put(y1).put(z1).put(r).put(g).put(b).put(a).put(s1).put(t1);
        vertices.put(x1).put(y2).put(z1).put(r).put(g).put(b).put(a).put(s1).put(t2);
        vertices.put(x2).put(y2).put(z1).put(r).put(g).put(b).put(a).put(s2).put(t2);

        vertices.put(x1).put(y1).put(z1).put(r).put(g).put(b).put(a).put(s1).put(t1);
        vertices.put(x2).put(y2).put(z1).put(r).put(g).put(b).put(a).put(s2).put(t2);
        vertices.put(x2).put(y1).put(z1).put(r).put(g).put(b).put(a).put(s2).put(t1);
    ...
    public void setupShaderProgram() {
        ...
        vertexShader = Shader.loadShader(GL_VERTEX_SHADER, "resources/default3d.vert");
    ...
    {
        int posSize = 3, colorSize = 4, textureSize = 2;

        /* Specify Vertex Pointer */
        int posAttrib = program.getAttributeLocation("position");
        program.enableVertexAttribute(posAttrib);
        program.pointVertexAttribute(posAttrib, posSize, 8 * Float.BYTES, 0);

        /* Specify RGBAColor Pointer */
        int colAttrib = program.getAttributeLocation("color");
        program.enableVertexAttribute(colAttrib);
        program.pointVertexAttribute(colAttrib, colorSize, 8 * Float.BYTES, posSize * Float.BYTES);

        /* Specify Texture Pointer */
        int texAttrib = program.getAttributeLocation("texcoord");
        program.enableVertexAttribute(texAttrib);
        program.pointVertexAttribute(texAttrib, textureSize, 8 * Float.BYTES, (posSize + colorSize) * Float.BYTES);
    }
