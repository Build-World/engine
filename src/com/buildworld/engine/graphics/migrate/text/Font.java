package com.buildworld.engine.graphics.migrate.text;
...
import com.buildworld.engine.graphics.migrate.bootstrap.Texture;
import com.buildworld.engine.graphics.migrate.bootstrap.renderer.Renderer;
import com.buildworld.engine.graphics.migrate.bootstrap.renderer.Renderer3D;
...
public class Font {
    ...
    public void drawText(Renderer renderer, CharSequence text, float x, float y, RGBAColor c) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            ((Renderer3D)renderer).drawTextureRegion(texture, drawX, drawY, 0, g.x, g.y, g.width, g.height, c);
            drawX += g.width;
        }
        renderer.end();
    }
