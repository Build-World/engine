...
public class InstancedMesh extends Mesh {
    ...
    public void renderListInstanced(List<Renderable> renderables, Transformation transformation, Matrix4f viewMatrix) {
        renderListInstanced(renderables, false, transformation, viewMatrix);
    }

    public void renderListInstanced(List<Renderable> renderables, boolean billBoard, Transformation transformation, Matrix4f viewMatrix) {
        initRender();
        ...
        for (int i = 0; i < length; i += chunkSize) {
            ...
            List<Renderable> subList = renderables.subList(i, end);
    ...
    private void renderChunkInstanced(List<Renderable> renderables, boolean billBoard, Transformation transformation, Matrix4f viewMatrix) {
        ...
        for (Renderable renderable : renderables) {
            ...
            if (text != null) {
                int col = renderable.getTextPos() % text.getNumCols();
                int row = renderable.getTextPos() / text.getNumCols();
            ...
            this.instanceDataBuffer.put(buffPos, billBoard ? renderable.getScale() : renderable.isSelected() ? 1 : 0);
        ...
        glDrawElementsInstanced(
                GL_TRIANGLES, getVertexCount(), GL_UNSIGNED_INT, 0, renderables.size());
